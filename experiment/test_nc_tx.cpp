 ...
 *
 *  This file is used to test transmitting NC-OFDM PHY frames over the air.
 */
...
void test_nc_tx(double freq, double sample_rate, double tx_gain, double amp, Rate phy_rate);
...
int main(int argc, char * argv[]){
    ...
    test_nc_tx(freq, sample_rate, tx_gain, amp, phy_rate);

	return 0;
}

/*!
 * \brief Function for testing the wno_ofdm transmitter
...
void test_nc_tx(double freq, double sample_rate, double tx_gain, double amp, Rate phy_rate)
{
    ...
    nc_transmitter tx = nc_transmitter(SC_MAP, freq, sample_rate, tx_gain, amp);
    ...
    while(1)
    {
        tx.send_frame(payload, phy_rate);
    }
